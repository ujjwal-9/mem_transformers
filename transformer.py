...
class Encoder(nn.Module):
    ...
    def __init__(self, encoder_layer, encoder_layer_final, number_of_layers, mem_size, model_dimension):
        ...
        self.src_memory = nn.init.xavier_uniform_(nn.Parameter(torch.empty(mem_size, model_dimension)))
...
#         intr_memory = intr_memory.mean(axis=0).unsqueeze(dim=0)
...
#         memory = memory.mean(axis=0).unsqueeze(dim=0)
        return hidden_state
