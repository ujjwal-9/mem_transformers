...
class Transformer(nn.Module):
    ...
    def __init__(self, batch_size, model_dimension, src_vocab_size, trg_vocab_size, number_of_heads, number_of_layers, dropout_probability, log_attention_weights=False, mem_size=16):
        super().__init__()
        ...
        decoder_layer = DecoderLayer(model_dimension, dropout_probability, mha, pwn, update_attn_src, output_attn_src, cross_attn)
        decoder_layer_final = DecoderLayer(model_dimension, dropout_probability, mha, pwn, update_attn_src, output_attn_src, cross_attn, True)
    ...
    def initialize_memory(self, bsz, mem_size, model_dimension):
        self.src_memory = nn.init.xavier_uniform_(nn.Parameter(torch.empty(bsz, mem_size, model_dimension)))
        self.trg_memory = nn.init.xavier_uniform_(nn.Parameter(torch.empty(bsz, mem_size, model_dimension)))
    ...
    def forward(self, src_token_ids_batch, trg_token_ids_batch, src_mask, trg_mask, init_mem=False):
        if init_mem:
            self.batch_size = src_token_ids_batch.shape[0]
            self.initialize_memory(self.batch_size, self.mem_size, self.model_dimension)
    ...
    def encode(self, src_memory, src_token_ids_batch, src_mask):
        ...
        # self.src_memory = nn.Parameter(src_memory_update)
        return hidden_state
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, model_dimension, multi_headed_attention, pointwise_net, update_attention, output_attention, apply_memory_update=False):
        super().__init__()

        self.src_multi_headed_attention = multi_headed_attention
        self.pointwise_net = pointwise_net
        self.norm = AddNorm(model_dimension)
        self.norm_final = AddNorm(model_dimension)
        
        self.apply_memory_update = apply_memory_update
...
class AddNorm(nn.Module):
    ...
    def forward(self, x, sublayer_output=None):
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, model_dimension, multi_headed_attention, pointwise_net, update_attention, output_attention, cross_attention, apply_memory_update=False):
        super().__init__()
        ...
        self.norm = AddNorm(model_dimension)
        self.norm2 = AddNorm(model_dimension)
        self.norm3 = AddNorm(model_dimension)
        self.norm_final = AddNorm(model_dimension)
        
        self.apply_memory_update = apply_memory_update
        if apply_memory_update:
            self.update_attention = copy.deepcopy(update_attention)
            self.output_attention = copy.deepcopy(output_attention)
            self.cross_attention = copy.deepcopy(cross_attention)

        self.model_dimension = model_dimension

    def forward(self, hidden_state, trg_memory, trg_representations_batch, trg_mask):
        ...
        if self.apply_memory_update:
            ...
            trg_representations_batch = self.norm2(decoder_hidden_state, trg_representations_batch)
        
        trg_representations_batch = self.cross_attention(hidden_state, decoder_hidden_state, None)
        trg_representations_batch = self.norm3(trg_representations_batch)
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, model_dimension, number_of_heads, dropout_probability, log_attention_weights=False):
        super().__init__()
