...
class Transformer(nn.Module):
    ...
    def forward(self, src_token_ids_batch, trg_token_ids_batch, src_mask, trg_mask, infer=False):
        self.hidden_state = self.encode(self.src_memory, src_token_ids_batch, src_mask)
        if infer:
            return self.hidden_state
        trg_log_probs = self.decode(self.hidden_state, self.trg_memory, trg_token_ids_batch, trg_mask)
        return self.hidden_state, trg_log_probs
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, model_dimension, multi_headed_attention, pointwise_net, update_attention, output_attention, cross_attention, apply_memory_update=False):
        ...
        if apply_memory_update:
            ...
            self.norm_attn = AddNorm(model_dimension)
